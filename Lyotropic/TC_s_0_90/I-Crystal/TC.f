...
c$$$!----------Global I2,I4, Q4 and Q6
           call Qdglobal(Q6glob,Q4glob)
           call Idglobal(I2glob,I4glob)
c$$$!--------------------------------
c$$$
!----------Local I4
!           call I4local()
...
!-------------------------------------------------------------------------
      ...
      do i = 1,NA
        do j = 1,Nb(i)
          ...
          do m = -4,4
            sqi = sqi + Cabs(I4(m,i))**2.0d0
            sqj = sqj + Cabs(I4(m,neindex))**2.0d0
            realdp = realdp + real(I4(m,i))*real(I4(m,neindex))
            imagdp = imagdp + aimag(I4(m,i))*aimag(I4(m,neindex))
          enddo
          scalarI4 = (realdp+imagdp)/((sqi**0.5)*(sqj**0.5))
!          sqi =0.0
!          sqj = 0.0
!          realdp = 0.0
!          imagdp = 0.0
