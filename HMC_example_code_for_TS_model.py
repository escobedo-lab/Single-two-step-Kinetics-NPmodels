...
lnvol_max = 0.00960791907074        # Maximum log volume displacement
...
nstar_min = 60            # Minimum cluster size for the sampling window [molecules
nstar_max = int(nstar_min+300)          # Maximum cluster size for the sampling window [molecules]
...
#        os.system(cmd1)
	...
	print(nstar_Z,nstar_G,'Lclus and Dual Lclus1 and 2')
	#sys.exit(-1)
#OP.write('%s\t%s\n'%(0,nstar))
	...
	local_zeo=open('local_clus_zeo.dat','w')
...
#thermo.write('%d\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n'%(0,nstar,vol,pe*(KT),hmc_acc,vol_acc,hmc_acc_w,vol_acc_w,dt,lnvol_max))
	...
	f2=open('Nclus_GYR.dat','r')

	for line in f2:
		CL3=line.split()[0]
		CL4=line.split()[1]
		local_gyr.write('%s\t%s\t%s\n'%((0),(CL3),(CL4)))
	...
	nstar_G_restart=nstar_G
	nstar_Z_restart=nstar_Z

	thermo.write('%d\t%d\t%s\t%s\t%f\t%f\t%f\n'%(0,nstar_Z,(nstar_G),pe*(KT),vol,dt,lnvol_max))
	thermo.flush()
	local_gyr.flush()
	local_zeo.flush()
...
#	if(int):
	...
	if(pickmove<=p_hmc):
#		print('NVT MOVE')
		if(abs((ke*KT)-30969.7102532448)>0.0000001):
...
#			x_new[i]=scalef*x[i]
		...
		#WRITING A NEW NPT1.lmp FILE: NOTE in 2023 version you can set image flags to 0
			...
			config.write('Masses\n')
			config.write('\n')
			config.write('1 28.0855\n')
			config.write('2 28.0855\n')
			config.write('\n')
...
#	print(nstar,'nstar')
	if comm.rank == 0:
#		print(nstar_Z,'nstar')
		if((isweep+1)>10000 and nstar_Z>=nstar_min+20 and cross_count==0):
			cross_count=1
			if(cross_count==1):
...
#		thermo.write('%d\t%d\t%s\t%s\t%s\t%s\t%s\t%s\n'%(0,nstar,hist[0],hist[1],nstar_dual,pe*(KT),dt,lnvol_max))
			thermo.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n'%((isweep+1),nstar_G,(nstar_Z),pe*(KT),vol,dt,lnvol_max,hmc_acc_w,hmc_acc,vol_acc,vol_acc_w))
#		thermo.flush()
