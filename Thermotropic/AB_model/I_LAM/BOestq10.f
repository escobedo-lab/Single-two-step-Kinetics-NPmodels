      program BOest
      include 'BOmetersq10.h'
      character*30 RIN,AXESIN
      integer i,k
      integer lenstar,count_1
      integer totOrdstar
      integer totOrdavg
      integer N4,id_4N(100000)
      double precision fracOrdstar
      double precision fracOrdavg
      double precision fracgrCstar
      double precision fracgrCavg,HN(3,3)
      include 'BOcoordsq10.h'
      RIN = 'coord.inp'
      id_4N(:)=0
!      open(unit=123,file='4N_num.dat')
!      read(123,*) NA_actual
!      write(*,*) NA_actual,'4N'
        NA_actual=NA
      !  write(*,*) NA_actual
		...
		read (1,*) HN(:,1)
                ...
                count_1=0
                 do i=1,16337
                        ...
                        count_1=count_1+1
         !             write(*,*) i
                        read(1,*) r(1,count_1),r(2,count_1)
     &,r(3,count_1)
                        else
                        read(1,*)
                        end if
        ...
        close(1)
        if(count_1.ne.NA)then
                write(*,*) 'NOT PSOOIBLE-q8'
                STOP
        end if
...
!      call getallneighborClus()
      ...
      open(unit = 21, File = "id_screen_q10.dat")
      open(unit=22,file='id_screen_num_q10.dat')
!do i=1,NA
! 	write(20,*)  qmavg(i)
! enddo
! close(20)
!      open(unit=19,file='4N_ids.dat')
!       do i=1,NA_actual
!         read(19,*) id_4N(i)
!       end do
!        close(19)
!      open(unit=20,file='coordTclus.inp')
!        write(20,*) 
!                write(20,*) CL(1),0.0,0.0
!                write(20,*) 0.0,CL(2),0.0
!                write(20,*) 0.0,0.0,CL(3)

        k=0
       do i=1,NA_actual
!         do k=1,N4
      !          write(*,*) qmavg(i)
                if(qmavg(i).gt.0.020)then
                k=k+1
!                write(20,*) r(1,i),r(2,i),r(3,i)
                write(21,*) 1
                else
                write(21,*) 0
                end if
...
!      flush(20)
      ...
      ! NOT REQUIRED FOR q8
c$$$      lenstar = 0 
c$$$      call getqmstar(lenstar)
c$$$      Open(unit = 20, File = "qmstar")
c$$$      do i = 1,lenstar
c$$$         write(20,*) qmstar(i)
c$$$      enddo
c$$$      close(20)
c$$$
c$$$      ! Ncon based on qmstar
c$$$      call getNConnectstar() 
c$$$      Open(unit = 20,File = "Nconstar")
c$$$      do i = 1,NA
c$$$         write(20,*) Nconstar(i)
c$$$      enddo
c$$$      close(20)
c$$$
c$$$      ! total number of ordered particles based on epsstar
c$$$      totOrdstar = 0
c$$$      do i = 1,NA
c$$$         if (Nconstar(i) .ge. epsstar) then
c$$$            if (epsMAX .gt. 0) then
c$$$               if (Nconstar(i) .le. epsMAX) then
c$$$                  totOrdstar = totOrdstar +  1
c$$$               endif
c$$$            else
c$$$               totOrdstar = totOrdstar +  1
c$$$            endif
c$$$         endif
c$$$      enddo
c$$$      fracOrdstar = totOrdstar/real(NA)
c$$$      !write(*,*) "Total Ordered star",totOrdstar,fracOrdstar
c$$$
c$$$      ! greatest cluster
c$$$      call getGreatestClusstar()
c$$$      fracgrCstar = grClusstar/real(NA)
c$$$      !write(*,*) "GreatestOrd star",grClusstar,fracgrCstar
      ! NOT REQUIRED FOR q8
...
!fac3=(4pi/2l+1)
        ...
        fac101(-10)=8.2878600121e-10
        fac101(-9)=3.7064436750e-09
        fac101(-8)=2.2848053291e-08
        fac101(-7)=1.6789821654e-07
        fac101(-6)=1.3845241623e-06
        fac101(-5)=1.2383560574e-05
        fac101(-4)=1.1748077086e-04
        fac101(-3)=1.1630002963e-03
        fac101(-2)=1.1860322411e-02
        fac101(-1)=1.2325608606e-01
        fac101(0)=1.2927207365e+00
        fac101(1)=1.2325608606e-01
        fac101(2)=1.1860322411e-02
        fac101(3)=1.1630002963e-03
        fac101(4)=1.1748077086e-04
