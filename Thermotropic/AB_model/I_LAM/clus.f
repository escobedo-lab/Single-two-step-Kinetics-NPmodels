...
!         FIN='input.dat'
!         SEED=4234212
        open(unit=123,file='id_screen_num.dat')
        read(123,*) NA_ref
        !write(*,*) NA_ref,'NA_ref'
!        close(123)
        if(NA_ref.eq.0)then
           !open(unit=107, file='Lclus_ZEO.dat')
!            OP= grClusQ6
           open(unit=107,file='coord_clus_num.dat') 
           
           write(107,*) 0
           flush(107)
!           open(unit=02,file='coord_clus_num.dat')

              STOP
        end if
        NA_actual=NA
        open(unit=124,file='id_screen.dat')
        do i=1,NA_ref
              read(124,*) qmavg(i)
        end do
...
!-------------------------------------------------------------------------
      ...
      integer Nq2A,idq2A(1000000)
      integer Nq2B,idq2B(1000000)  
      include 'coords.h'
      sqi =0.0
      sqj =0.0
      realdp = 0.0
...
!        write(*,*) rsq_new
!
        open(unit=501,file='id_screen_num_q2A.dat')
        read(501,*) Nq2A

        open(unit=502,file='id_screen_q2A.dat')
        idq2A(:)=0
        do i=1,6912
            read(502,*) idq2A(i)
        end do
        ...
        open(unit=5010,file='id_screen_num_q2B.dat')
        read(5010,*) Nq2B

        open(unit=5020,file='id_screen_q2B.dat')
        idq2B(:)=0
        do i=1,6912
            read(5020,*) idq2B(i)
        end do
...
!        open(unit=3001,file='NB_zeo_final.dat')
        open(unit=300,file='id_TZ.inp')
        open(unit=301,file='coord_clus_final.inp')
        open(unit=302,file='coord_clus_num.dat')
        open(unit=3002,file='Lclus_all.dat')
...
!        open(unit=23,file='con.dat') 
      do i = 1,NA_actual
        ...
        if(i.le.6912)then
              if(qmavg(i).eq.1.and.idq2A(i).eq.1)then
                write(300,*) 1
                tcheck=1
              ...
              if(qmavg(i).eq.1.and.idq2B(i-6912).eq.1)then
                write(300,*) 1
                tcheck=1
                write(301,*) r(1,i), r(2,i), r(3,i)
              else
