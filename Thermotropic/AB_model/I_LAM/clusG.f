         program hexagonalprisms 
c Confinement Code GJK 
         include 'parametersG.h'
     ...
     & pressstart,pressstep, c1(3,1),rsq
         integer Ntransatt,Nrotatt,Nvolatt,i,j,Nmoveadj,fileindex
         integer flag3, Nattpl, Nsuccpl,count_neigh
         !integer N_ZEO,t_check
!         double precision rG(3,NA),qG(3,NA),HNG(3,3),qij(3),rij(3)
         real ran2,pickmove	
!------------------------------------------------------------	
!        US variables
!         integer OP_right,OP_left
         integer isCROSS_right, isCROSS_left !for next window 
         integer NsaveOP, NA_ref3
         integer Ntrck_OP
...
!         FIN='input.dat'
!         SEED=4234212
        open(unit=123,file='id_screen_num_q8.dat')
        read(123,*) NA_ref1
        !write(*,*) NA_ref1,'NA_ref q8'
        close(123)
        open(unit=1230,file='id_screen_num_q10.dat')
        read(1230,*) NA_ref2
        !write(*,*) NA_ref2,'NA_ref q10'
        close(1230)

        open(unit=12300,file='coord_clus_num.dat')
        read(12300,*) NA_ref3
        !write(*,*) NA_ref2,'NA_ref q10'
        close(12300)


        if(NA_ref1.eq.0.and.NA_ref2.eq.0)then
           open(unit=107, file='Lclus.dat')
!            OP= grClusQ6
           write(107,*) 0
           flush(107)
              STOP
        ...
        if(NA_ref3.eq.0)then
           open(unit=1070, file='Lclus.dat')
!            OP= grClusQ6
           write(1070,*) 0
           flush(1070)
              STOP
        ...
        NA_actual=NA
        open(unit=124,file='id_screen_q8.dat')
        qmavg1(:)=0
        do i=1,4247
              read(124,*) qmavg1(i)
        end do

        open(unit=1240,file='id_screen_q10.dat')
        qmavg2(:)=0
        do i=1,4247
              read(1240,*) qmavg2(i)
        end do
        ...
        open(unit=125,file='Lclus_all.dat')
        read(125,*) N_ZEO
        close(125)

!        if(N_ZEO.eq.0)then
!                STOP 'NOT POSSIBLE ZEO- SHOULD NOT REACH HERE HERE'
!        end if
        if(N_ZEO.ne.0)then
        open(unit=1250,file='id_TZ.inp')
        do i=1,12090
               read(1250,*) ID_TZ(i)
                !write(*,*) ID_TZ(i)
        end do
...
!         open (unit=36,file='s0.9.txt')
!            
!            do m = 1, 24
!                read(36,*) VU(1,m), VU(2,m), VU(3,m)
!            end do
c----------------------------------------------------------
!          write(*,*) 'REACHED HERE'
         open (unit=10,file='coord.inp')
!          write(*,*) 'REACHED 2'
           read (10,*) HN(:,1)
           read (10,*) HN(:,2)
           read (10,*) HN(:,3)
             do i=1,NA_actual
                 read(10,*) r(1,i),r(2,i),r(3,i)
             enddo
             close(10)
!            write(*,*) HN
             call METRICN
...
!                        if(abs(q(j,i)).gt.0.50d0)then

!                                STOP 'IMPOSSIBLE OUTSIDE THE BOX'
...
!                        end if
            ...
            enddo
                ...
                q(:,:)=q(:,:)-dnint(q(:,:))
!         write(*,*) 'REACHED S'
!         open (unit=2,file='coordS.inp')
!         write(*,*) 'REACHED S1' 
!           read (2,*) HNG(:,1)
!           read (2,*) HNG(:,2)
!           read (2,*) HNG(:,3)
!             do i=1,4247
!                 read(2,*) rG(1,i),rG(2,i),rG(3,i)
!             enddo
!             close(2)

!                do i=1,4247
!                do j=1,3
!                    qG(j,i)=HNI(j,1)*rG(1,i)+HNI(j,2)*rG(2,i)
!     & +HNI(j,3)*rG(3,i)

!                        if(abs(qG(j,i)).gt.0.50d0)then
!
!                                STOP 'IMPOSSIBLE OUTSIDE THE BOX'
!
!                        end if
...
!                do j= 1,NA
!                     if(i.ne.j)then
!                        dist=sqrt((r(1,i)-r(1,j))**2.0
!     &+(r(2,i)-r(2,j))**2.0+(r(3,i)-r(3,j))**2.0)
...
!              do j=1,N_ZEO
!LOOP OVER T PARTICLES BELONGING TO ZEOLITE                

!                if(i.ne.ID_TZ(j))then
!                ID_TZ(i)
!CHECK for q8 neighbor
!                  do l=1,NA_ref1
!                        qij(:)=q(:,i)-q(:,qmavg1(l))
!                        qij(:)=qij(:)-nint(qij(:))
!                         do k=1,3
...
!                        if (rsq.lt.(5.70d0)**2.0) then
!                                t_check=1
!                        end if
!l loop                     
!                  end do

!IF ABOVE IS NOT SATISFIED THEN CHECK IN q10

!                if(t_check.eq.0)then

!CHECK for q10 neighbor
!                  do l=1,NA_ref2
!                        qij(:)=q(:,i)-q(:,qmavg2(l))
!                        qij(:)=qij(:)-nint(qij(:))
!                         do k=1,3
...
!                allClus=allClus+t_check
!i toop
!          end do


!           open(unit=107, file='LclusG.dat')
!            OP= grClusQ6
!           write(107,*) allClus
!           flush(107)
         
!           close(107) 
!          open(230,file='Nclus.dat')
!          write(*,*) cStcount
!          cStTMP = cSt(NA_actual+1)
!          write(230,*) 0,int(cStTMP)
!         do i=1,NA_actual
!           if (cSt(i).ne.0) then
!              cStTMP = cSt(i)
!              write(230,*) i,int(cStTMP)
!           endif
!         enddo
         !write(*,*) cSt(0)
...
!         Ntrack=0
        ...
        if (steps.gt.Neq) then
           ...
           if (grClusQ6.ge.MAXCLUS.and.mod(steps,Nrecord).eq.0) then
              MAXCLUS = grClusQ6
...
c-----------------------FROM GJK CODE------------------------------------------------
         ...
         include 'parameters.h'
         double precision qij(3),rij(3), c1(3,2), VR(3,24)
         double precision vpart1(3,24),vpart2(3,24), vecz
...
c------------------CELL CHECK FROM GJK CODE-----------------------------
        ...
        subroutine cellcheck(pickmol,lcell,flag)
         ...
         double precision vpart1(3,24),vpart2(3,24)
...
c            write(*,*) rsq
            ...
            if(rsq.le.rsq_new) then
!              neighbor1(append1,i) = j
              append1 = append1+1
            endif

!            if(rsq.le.rcutsqnb2) then
!              neighbor2(append2,i) = j
!              append2 = append2+1
!            endif
...
!---------------------------------------------------------
     ...
     &            HN(k,3)*qij(3)
          ...
          Theta = DATan2(Dsqrt(rij(1)**2.0d0 + rij(2)**2.0d0),rij(3))   
...
!-------------------------------------------------------------------------
      ...
      include 'parametersG.h'
      integer i,j,m,neindex,tcheck,CN_check
      double precision sqi,sqj,realdp,imagdp,scalar
      double precision qij(3),rij(3),dist
      include 'coordsG.h'
      sqi =0.0
      sqj =0.0
      realdp = 0.0
      ...
      do  i = 1,NA_actual
        NconQ6(i) = 0
      enddo  
!HERE THE Q6 clustering is modified for Diamond- exluding AA BB
!neighbors
!        open(unit=1214,file='id4.dat')
        open(unit=3001, file='id_TZ1.inp')
        allClus=0
        open(unit=301,file='coord_clus_final.inp')
        open(unit=302,file='coord_clus_num.dat')

        write(301,*) HN(1,1), HN(2,2), HN(3,3)

!        open(unit=23,file='con.dat') 
!        write(*,*) N_ZEO,'NZEO'
!        write(*,*) ID_TZ(1:N_ZEO)
        CN_check=0
        if(N_ZEO.eq.0)then
...
!
!         if(neindex.gt.12090)then

!           if(qmavg1(neindex-12090).eq.1.or.
!     &qmavg2(neindex-12090).eq.1)then
!            NconQ6(i) =NconQ6(i)+1
!                write(*,*) '------'
!                write(*,*) neindex,i
!                write(*,*) 'satisfied',qmavg2(neindex-12090)
!                write(*,*) '------'
!                tcheck=1
!           endif
!
...
!        end do
!
!         allClus=allClus+tcheck
!         if(tcheck.eq.1)then
!                write(301,*) r(1,i), r(2,i), r(3,i)
!         end if
          write(3001,*) 0 
!
       end do
...
!                write(1214,'(1x,i0)',advance='no') i
           ...
           allClus=allClus+tcheck
!end do for i after else
        end do
!N_ZEO
        end if

        if(N_ZEO.ne.0.and.allClus.eq.0)then
                open(unit=1290,file='Lclus.dat')
                write(1290,*) 0
                close(1290)
        end if
...
!      allClus = 0
      ...
      ! write(*,*) OP_right, OP_left,grClusQ6, 'LEFT RIGHT'
       !write(*,*) histQ6 
      !  write(*,*) NA_actual
...
!compute for zero only if the left window is zero
       if(OP_left.eq.0)then
!Just Total -total clustersize
         cSt(NA_actual+1) = cSt(NA_actual+1) + NA_actual - allClusQ6
       end if
        cStcount = cStcount + 1
!end if for grclus condition
      end if
      !write(*,*) cSt(1)
!        open(unit=1212,file='ids.dat')
!      do i=1,NA_actual
!            if(belongsto(i).gt.0)then
!                write(1212,'(1x,i0)',advance='no') i
!            end if
!      end do
