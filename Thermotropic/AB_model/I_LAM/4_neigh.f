...
c Confinement Code GJK 
     ...
     & pressstart,pressstep, c1(3,1),qij(3),rij(3),rsq
         integer Ntransatt,Nrotatt,Nvolatt,i,j,Nmoveadj,fileindex
         integer flag3, Nattpl, Nsuccpl,count_neigh,count_num
         real ran2,pickmove	
!------------------------------------------------------------	
!        US variables
...
c----------------------------------------------------------
         ...
         open (unit=1,file='coord.inp')
           ...
           read (1,*) HN(:,3)
             do i=1,NA
                 read(1,*) r(1,i),r(2,i),r(3,i),atype(i)
             enddo
             close(1)
!            write(3,*) 'NA = ', NA
!            write(3,*)
...
!        open(unit=223,file='coord4N.inp')
!        open(unit=224,file='4N_num.dat')
!        open(unit=225,file='4N_ids.dat')
!           write (223,*) HN(1,1),HN(2,2),HN(3,3)
           !write (223,*) HN(:,2)
           !write (223,*) HN(:,3)
!            count_num=0
!           do i=1,NA
...
!
!                        if(rsq.le.(4.0d0)**2.0)then
!                               count_neigh=count_neigh+1

!                        end if
!                     end if
!                end do
!                 if(count_neigh.ne.4)then
!                        write(223,*) r(1,i),r(2,i),r(3,i)
!                        write(225,*) i
...
!-------------------------------------------------------------------------        
      ...
      integer icell,i,j,append,JCELL0,NABOR,JCELL
       integer append1,append2,count_num
c      integer lengr
      include 'coords4N.h'
...
!         if(atype(i).eq.atype(neindex))then
!              tcheck=tcheck+1
!              if(tcheck.gt.2)then
!                NconQ6(i)=0
!                goto 150
...
!          end if
     ...
     &            HN(k,3)*qij(3)
          enddo
         dist=sqrt(rij(1)**2.0+rij(2)**2.0+rij(3)**2.0)
        ...
        do j=1,Nb(i)
          ...
          imagdp = 0.0
          do m = -6,6
            sqi = sqi + Cabs(q6(m,i))**2.0d0
            sqj = sqj + Cabs(q6(m,neindex))**2.0d0
            realdp = realdp + real(q6(m,i))*real(q6(m,neindex))
            imagdp = imagdp + aimag(q6(m,i))*aimag(q6(m,neindex))
          enddo
          scalar = scalar+(realdp+imagdp)/((sqi**0.50d0)*(sqj**0.50d0))
!          if(scalar>dcQ6) then
!            NconQ6(i) =NconQ6(i) +1
!          endif          
        enddo

          scalar=scalar/real(Nb(i))
          write(23,*) Nb(i)
